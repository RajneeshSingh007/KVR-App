...
class NotificationVM : ViewModel() {
    private var _state = MutableStateFlow<Response<Notification>>(Response.Empty())
    val state = _state.asStateFlow()

    fun fetchNotify() = viewModelScope.launch {
        _state.value = Response.Loading(true)
        try{
            val result  = ApiServices.getNotificationApi()
